...
const selectionDecorations = new Map<string, vscode.TextEditorDecorationType>();

export function activate(context: vscode.ExtensionContext) {
  context.subscriptions.push(
    vscode.commands.registerCommand("webrtcCollab.start", async () => {
...
async function openPanel(context: vscode.ExtensionContext) {
  ...
  const recv = panel.webview.onDidReceiveMessage(async (msg: any) => {
    ...
    if (msg.type === "presence-leave") {
      ...
      if (cd) { cd.dispose(); cursorDecorations.delete(msg.id); }
      const sd = selectionDecorations.get(msg.id);
      if (sd) { sd.dispose(); selectionDecorations.delete(msg.id); }
      return;
    }
  });
  subs.push(recv);
...
function cleanup() {
  ...
  for (const d of cursorDecorations.values()) { try { d.dispose(); } catch {} }
  for (const d of selectionDecorations.values()) { try { d.dispose(); } catch {} }
